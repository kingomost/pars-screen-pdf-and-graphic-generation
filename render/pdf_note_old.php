...
<page backimg="<?=DIR.DIR_SEPARATOR.'template'.DIR_SEPARATOR?>code_0.jpg" backimgw="500%" style="font-family: freesans">
		<br><br><br>
		<h1 style="text-align: center; font-family: freesans;">
			<!---<?=$arr_info['site'][100].': '?>--->
			<br>
			<b><?=$model_obj->domain?></b> (<?=date('d.m.Y', $model_obj->time_create)?>)
		</h1>
		<br><br><br>
		<div style="width: 90%; margin-left: 30px;">
			<?php
			if (is_file(DIR.DIR_SEPARATOR.$model_obj->scrin_nout) && is_file(DIR.DIR_SEPARATOR.$model_obj->scrin_smart)) {
			?>
				<img src="<?=DIR.DIR_SEPARATOR.$model_obj->scrin_nout?>" style="background-color: transparent; border: none; width: 70%">
				<img src="<?=DIR.DIR_SEPARATOR.$model_obj->scrin_smart?>" style="background-color: transparent; border: none; width: 29%">
			<?php
			} elseif (is_file(DIR.DIR_SEPARATOR.$model_obj->scrin_nout) && !is_file(DIR.DIR_SEPARATOR.$model_obj->scrin_smart)) {
			?>
				<img src="<?=DIR.DIR_SEPARATOR.$model_obj->scrin_nout?>?>" style="background-color: transparent; border: none; width: 95%">
			<?php
			} else {
			?>
				<img src="<?=DIR.DIR_SEPARATOR.$model_obj->scrin_smart?>" style="background-color: transparent; border: none; width: 95%">
			<?php
			}
			?>
		</div>
...
<div style="width: 100%;">
				...
				<br>
				<h4><b><?=$arr_info['contents_title'][200].': '?></b><i><?=(is_string($model_obj->title) && $model_obj->title!=='')?$model_obj->title:'---'?></i></h4>
				<hr style="margin: -2px 0 -15px 0; padding: -2px 0 -15px 0;">
				<h4><b><?=$arr_info['contents_keywords'][200].': '?></b><i><?=(is_string($model_obj->keywords) && $model_obj->keywords!=='')?$model_obj->keywords:'---'?></i></h4>
				<hr style="margin: -2px 0 -15px 0; padding: -2px 0 -15px 0;">
				<h4><b><?=$arr_info['contents_description'][200].': '?></b><i><?=(is_string($model_obj->description) && $model_obj->description!=='')?$model_obj->description:'---'?></i></h4>
				<?php
					if (is_array($model_obj->headings) && count($model_obj->headings)>0) {
				?>
					<hr style="margin: -2px 0 -15px 0; padding: -2px 0 -15px 0;">
					<h4><b><?=$arr_info['contents_headings'][200].': '?></b></h4>
					<table style="width: 90%; border-collapse: collapse;">
					<?php
					foreach ($model_obj->headings as $key=>$value) {
...
<div style="width: 100%;">
		<?php
		$buf = $model_obj->all_link;
		$buf_img = $model_obj->grafic_link;
		//if (is_array($buf) && count($buf)>0 && 
...
<div style="width: 100%;">
		...
		<?php
		if ($model_obj->html_version!==false && $model_obj->html_version!=='' && !is_null($model_obj->html_version)) {
		?>
			<hr style="margin: -2px 0 -15px 0; padding: -2px 0 -15px 0;">
			<h4><b><?='HTML: '?></b><i><?=$model_obj->html_version?></i></h4>
		<?php
		}
		?>
		
		<?php
		if ($model_obj->robots_txt!==false && $model_obj->robots_txt!=='' && !is_null($model_obj->robots_txt)) {
		?>
			<hr style="margin: -2px 0 -15px 0; padding: -2px 0 -15px 0;">
			<h4><b><?='Robots.txt: '?></b><i><?=$model_obj->domain.'/robots.txt'?></i></h4>
		<?php
		}
		?>
		
		<?php
		$buf = $model_obj->site_map;
		if (is_array($buf) && count($buf)>0) {
		?>
			<hr style="margin: -2px 0 -15px 0; padding: -2px 0 -15px 0;">
			...
			?>
			<tr>
				<td style="text-align: center; padding: 0 5px 0 15px; border: 1px solid #e8ded5;" colspan="2"><b><?=$key?></b></td>
			</tr>
				<?php
